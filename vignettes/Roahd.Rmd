...
```{r mfDataComponents, eval = TRUE, fig.width = 4, fig.height = 4, fig.align = 'center' }
...
```
```{r fData_subset_2, fig.align='center', fig.width=7, fig.height=4 }
...
grid $[t_0, t_1, \ldots, t_P] \subset I$ the sample estimator of covariance 
function is $C(t_i,t_j) = \widehat{Cov}(X(t_i),X(t_j))$ for $i,j=1,\ldots,P$. 
Given another univariate functional dataset $Y_1, Y_2, \ldots, Y_N$, the sample 
estimator of the cross-covariance function is 
$C_{X,Y}(t_i,t_j) = \widehat{Cov}(X(t_i),Y(t_j)$.
...
$C^{k,l} = [ C(t_i,t_j)]_{i,j=1}^{P}$, for $k,l = 1, \ldots, L$. 
Of course, it is $C^{k,l} = [C^{l,k}]^T$.
The cross-covariance between two multivariate datasets is given by the cross
covariances of the components.

When `X` is a univariate or multivariate functional dataset, and if `Y` is
...
of the first component with the second, etc. In the second case, the list of
cross-covariances between `X`'s components and `Y` are returned. In the third case,
the list of upper-triangular blocks of cross-covarainces between `X` and `Y` 
is returned.

```{r cov, collapse = TRUE, eval = FALSE}
...
```{r plot_cov, eval = FALSE, collapse = TRUE, fig.align = 'center', fig.height = 4, fig.width = 4}
  plot( C, main = 'Covariance function', xlab = 'time', ylab = 'time' )
```
...
```{r cor_kendall_2, eval = TRUE, collapse = TRUE, fig.align = 'center', fig.height = 4, fig.width = 6} 
